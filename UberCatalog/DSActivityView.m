...
@dynamic borderView, activityLabelLineBreakMode;
...
{
	return [self newPrivateActivityViewForView: addToView withLabel: labelText width: 0];
}
//**END_MINT_MOD**
...
{
	// Not autoreleased, as it is basically a singleton:
	return [[self alloc] initForView: addToView withLabel: labelText width: labelWidth privateToView:YES];
}
//**END_MINT_MOD**
...
*/
 ...
 //**BEGIN_MINT_MOD**  dsr added the privateToView: parameter here, and rewrote the beginning of this method to support it
- (DSActivityView *)initForView:(UIView *)addToView withLabel:(NSString *)labelText width:(NSUInteger)labelWidth privateToView:(BOOL)inPrivate;
{
	...
	//dsr mint - THIS CODE
	self.isPrivate=inPrivate;

	if(_isPrivate)
	{
		[addToView setPrivateActivityView:self];
	}
	else
	{
		[self removeView];
...
{
	return [self initForView:addToView withLabel:labelText width:labelWidth privateToView:NO];
}
...
//		[_originalView setPrivateActivityView:nil
//					  ifCurrentActivityViewIs:self];
	}
	_originalView=nil;
}
//**END_MINT_MOD**
...
{
	self.activityLabel.text = inText;
}

/* -----------------------------------------------------------------------------
	Written by			Date
	Dave Wright			06/19/12
 ------------------------------------------------------------------------------*/
- (NSLineBreakMode)activityLabelLineBreakMode
{
	return ( (nil != self.activityLabel) ? self.activityLabel.lineBreakMode : NSLineBreakByTruncatingTail );
}
...
/* -----------------------------------------------------------------------------
	...
	Dave Wright			06/19/12
 ------------------------------------------------------------------------------*/
- (void) setActivityLabelLineBreakMode:(NSLineBreakMode)inLineBreakMode
{
	if ( nil != self.activityLabel )
		self.activityLabel.lineBreakMode = inLineBreakMode;
}
//**END_MINT_MOD**
...
- (void)layoutSubviews;
{
    ...
    CGSize textSize = [self.activityLabel.text sizeWithAttributes:[NSDictionary dictionaryWithObject:[UIFont systemFontOfSize:[UIFont systemFontSize]] forKey:NSFontAttributeName]];
...
//**BEGIN_MINT_MOD**
// ----------------------------------------------------------------------------------------
#pragma mark Private ActivityView support
// ----------------------------------------------------------------------------------------

static NSString *kActivityViewKey=@"__ActivityView__";

+(DSActivityView *)currentPrivateActivityViewForView:(UIView *)inView
{
	DSActivityView *theActivityView;

	CALayer *theLayer=[inView layer];
		
	theActivityView=[theLayer valueForKey:kActivityViewKey];
	
	return theActivityView;
}
					  ...
					  forView:(UIView *)inTargetView;
{
	...
	CALayer *theLayer=[inTargetView layer];
	[theLayer setValue:inActivityView forKey:kActivityViewKey];
...
//**BEGIN_MINT_MOD added support for private views
- (UIView *)viewForView:(UIView *)view;
{
	if(!_isPrivate) //this behavior only applies to non-private activity views
	{
...
/* -----------------------------------------------------------------------------
    ...
    Revised by		Date		Summary of changes
    Jon Doyel		03/07/2011	[IPHN-837] Now accomodates status text that is more
                                    than one line.


    Positions and sizes the various views that make up the activity view,
...
- (void)layoutSubviews;
{
	...
	if (self.labelWidth > 10)
	{
		// Create a constraint CGSize that is limited in width to self.labelWidth, but has the
		//	the same height as our superview's frame (i.e. much bigger than we need) so that
		//	the calculated size is no wider than self.labelWidth.
		CGSize constraint = CGSizeMake(self.labelWidth,
		                               self.superview.frame.size.height);

		// Get the size for the text based on the constraint (this will ultimately be used just
		//	for the height calculation.
		textSize = [self.activityLabel.text boundingRectWithSize:constraint
														 options:NSStringDrawingTruncatesLastVisibleLine
													  attributes:[NSDictionary dictionaryWithObject:[UIFont boldSystemFontOfSize: [UIFont systemFontSize]] forKey:NSFontAttributeName]
														 context:nil].size;
		
		CGSize singleLineHeight = [@"Test" sizeWithAttributes:[NSDictionary dictionaryWithObject: [UIFont boldSystemFontOfSize: [UIFont systemFontSize]] forKey:NSFontAttributeName]];

		NSInteger rowCount = textSize.height / singleLineHeight.height;
		if (rowCount > 0)
		{
			self.activityLabel.numberOfLines = rowCount;
		}

		// Now restored the original width.
		textSize.width = self.labelWidth;
	}
	else
	{
		textSize = [self.activityLabel.text sizeWithAttributes:[NSDictionary dictionaryWithObject: [UIFont boldSystemFontOfSize: [UIFont systemFontSize]] forKey:NSFontAttributeName]];
	}
...
{
//**BEGIN_MINT_MOD**
	[self removeView]; //dsr call instance version instead of class version (private view support)
//**END_MINT_MOD**
}
...
+ (void)removeViewAnimated:(BOOL)animated;
{
	...
	if (animated)
		...
		[self removeView]; //dsr call instance version instead of class version
	//**END_MINT_MOD**
}
...
//**BEGIN_MINT_MOD**
@implementation UIView (DSActivityViewSupport)

-(DSActivityView *)currentPrivateActivityView;
{
	return [DSActivityView currentPrivateActivityViewForView:self];
}
-(void)setPrivateActivityView:(DSActivityView *)inActivityView;
{
	DSActivityView *currentView=[DSActivityView currentPrivateActivityViewForView:self];
	if(currentView)
		[currentView removeFromSuperview];
	
	[DSActivityView setPrivateActivityView:inActivityView
								   forView:self];
}
	  ...
	  ifCurrentActivityViewIs:(DSActivityView *)inOldActivityView;
{
	...
	if(currentView==inOldActivityView)
	{
		[DSActivityView setPrivateActivityView:inNewActivityView
									   forView:self];
	}
}
...
{
	DSActivityView *activityView=[self currentPrivateActivityView];
